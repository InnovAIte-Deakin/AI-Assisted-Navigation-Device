...
phone-performance preflight: CUDA being unavailable then produces `FAIL` and a
non-zero exit code. The command checks availability, not a specific GPU model;
...
- Python, torch, optional torchvision, CUDA build/availability, selected device,
  GPU name, and GPU memory where safe to retrieve.
...
- `FAIL`: a requested identity/backend check failed, the backend was malformed
  or unreachable, or CUDA was required but unavailable.
