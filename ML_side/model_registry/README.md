...
15. a portable registry reference to the exact committed comparison report
...
For the automatic route, the candidate record must also contain:

```json
{
  "automatic_promotion_evidence": {
    "promotion_report_reference": "ML_side/model_registry/promotions/<model>-comparison.json"
  }
}
```

The supplied `--promotion-report` must be that exact portable, repository-bound
file. This preserves the existing approved-policy `PASS` checks while giving
read-only release readiness durable evidence to validate later. A production
lifecycle alone never grants authorization.
