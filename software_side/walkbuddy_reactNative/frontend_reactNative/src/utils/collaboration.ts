...
class CollaborationService {
  ...
  private messageHandlers: Map<MessageType, ((msg: CollaborationMessage) => void)[]> = new Map();
  ...
  async getSessionStatus(sessionId: string): Promise<SessionStatus> {
    try {
      ...
      if (!this.validateSessionId(normalizedId)) {
        throw new Error(`Invalid session code format. Must be 8 alphanumeric characters. Got: "${sessionId}"`);
      }
      
      console.log(`[Collaboration] 📊 Checking status for session: "${sessionId}" → normalized: "${normalizedId}"`);
      ...
      if (!response.ok) {
        ...
        console.error(`[Collaboration] ❌ Status check failed: ${errorMessage}`);
        throw new Error(errorMessage);
      }
      const status = await response.json();
      console.log(`[Collaboration] ✅ Status: user=${status.user_connected}, guide=${status.guide_connected}`);
      return status;
    } catch (error) {
  ...
  connect(sessionId: string, role: CollaborationRole): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // Validate format
      if (!this.validateSessionId(normalizedId)) {
        const error = new Error(`Invalid session code format. Must be 8 alphanumeric characters. Got: "${sessionId}"`);
      ...
      console.log(`[Collaboration] Session ID: "${sessionId}" → normalized: "${normalizedId}"`);
      ...
      try {
        ...
        this.ws.onopen = () => {
          ...
          console.log(`[Collaboration] ✅ Socket connected (socket_id: ${this.socketId})`);
          console.log(`[Collaboration] ✅ Connected as ${role} to session "${normalizedId}"`);
        ...
        this.ws.onerror = (error) => {
          console.error(`[Collaboration] ❌ WebSocket error (socket_id: ${this.socketId}):`, error);
          reject(error);
        };

        this.ws.onclose = (event) => {
          console.log(`[Collaboration] 🔌 Socket disconnected (socket_id: ${this.socketId}, code: ${event.code}, reason: ${event.reason})`);
          this.ws = null;
          this.socketId = null;
  ...
  disconnect(): void {
    if (this.ws) {
      try {
        console.log(`[Collaboration] 🔌 Disconnecting (socket_id: ${this.socketId})`);
  ...
  sendCameraFrame(base64Image: string): void {
    if (this.role !== "user" || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
    ...
    try {
      ...
      const frameCount = (this as any).frameSendCount = ((this as any).frameSendCount || 0) + 1;
      if (frameCount % 8 === 0) { // ~8 FPS, so log once per second
        console.log(`[FRAME] 📤 Sent ${frameCount} frames (room: ${room}, bytes: ${imageData.length})`);
      }
    } catch (error) {
  ...
  sendGuidance(message: string): void {
    if (this.role !== "guide" || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
      return;
    }
  ...
  sendWebRTCOffer(sdp: RTCSessionDescriptionInit): void {
    if (this.role !== "user" || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.error("[Collaboration] Cannot send WebRTC offer: not connected as user");
      return;
    }
  ...
  sendWebRTCAnswer(sdp: RTCSessionDescriptionInit): void {
    if (this.role !== "guide" || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.error("[Collaboration] Cannot send WebRTC answer: not connected as guide");
      return;
    }
  ...
  sendWebRTCICE(candidate: RTCIceCandidateInit): void {
    ...
    try {
      const code = normalizeCode(this.sessionId || "");
      const room = roomFor(code);
      console.log(`[Collaboration] 📤 Sending WebRTC ICE candidate (room: ${room})`);
      this.ws.send(
        JSON.stringify({
          type: "webrtc_ice",
          candidate: candidate,
      ...
      console.error("[Collaboration] Error sending WebRTC ICE candidate:", error);
    }
  }
   ...
   */
  onMessage(type: MessageType, handler: (msg: CollaborationMessage) => void): () => void {
  ...
  private handleMessage(message: CollaborationMessage): void {
    ...
    console.log(`[Collaboration] 📨 Received message type: "${rawType}"`, message);
    ...
    if (handlers) {
      console.log(`[Collaboration] ✅ Found ${handlers.length} handler(s) for type "${msgType}"`);
      handlers.forEach((handler, index) => {
        try {
          console.log(`[Collaboration] 🔄 Calling handler ${index + 1} for "${msgType}"`);
          handler(message);
        } catch (error) {
          console.error(`[Collaboration] ❌ Error in message handler for ${msgType}:`, error);
        }
      });
    } else {
      console.warn(`[Collaboration] ⚠️ No handlers registered for message type "${msgType}"`);
      console.warn(`[Collaboration] ⚠️ Available handlers:`, Array.from(this.messageHandlers.keys()));
    }
    
    // Also call handlers for legacy type if different
    if (msgType !== message.type && message.type in this.messageHandlers) {
      const legacyHandlers = this.messageHandlers.get(message.type as MessageType);
      if (legacyHandlers) {
        legacyHandlers.forEach((handler) => {
          try {
            handler(message);
          } catch (error) {
            console.error(`Error in legacy message handler for ${message.type}:`, error);
          }
        });
      }
    }
