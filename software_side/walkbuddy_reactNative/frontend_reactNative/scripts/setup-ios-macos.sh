...
if echo "$IDENTITY_OUTPUT" | grep -q "0 valid identities found"; then
    ...
    echo ""
    echo "2. In Xcode:"
    echo "   Xcode -> Settings -> Apple Accounts (or Accounts on older Xcode versions)"
    echo ""
...
then
    ...
    ok "Basic code signing private-key access works."

    echo ""
    echo "NOTE: During the first full Xcode build, macOS may still"
    echo "request access to the Apple Development private key."
    echo "If prompted once, enter your login Keychain password and"
    echo "choose Always Allow."
    echo ""
    echo "If the same codesign Keychain dialog repeats continuously,"
    echo "stop the build with Ctrl+C and run:"
    echo ""
    ...
    echo "  \"$LOGIN_KEYCHAIN\""
    echo ""
    echo "Enter the login Keychain password locally when macOS asks."
    echo "Never paste or store that password in this script."
    echo "Then rerun: bash scripts/setup-ios-macos.sh"
...
echo "Basic codesign test: READY"
echo ""
...
echo ""
echo "This build installs the WalkBuddy development app on the connected iPhone."
echo ""
echo "Important: this native build does NOT start the Agora session server"
echo "or Cloudflare tunnels. If Ask-a-Friend later reports:"
echo "  EXPO_PUBLIC_AGORA_SESSION_URL is not configured"
echo "that is expected until the full development stack is started."
echo ""
echo "After this build succeeds, press Ctrl+C to stop Metro if needed."
echo "For normal WalkBuddy development, open a new Terminal and run:"
echo ""
echo "  cd \"$SOURCE_ROOT\""
echo "  npm run dev:call"
echo ""
echo "If you stay in this same Terminal instead, first run:"
echo ""
