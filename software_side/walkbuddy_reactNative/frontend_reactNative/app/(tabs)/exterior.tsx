...
import { metersBetween, updateStepIndex } from "../../src/utils/navigationHelpers";
  ...
  */
      ...
      locationSubscriptionRef.current = await Location.watchPositionAsync(
        ...
        async (loc) => {
          ...
          if (currentRoute && currentRoute.steps.length > 0) {
            ...
            // milestone / voice callouts (kept as-is from your file)
  ...
  const handleSearchRoute = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error.message || "Failed to find location. Please try again."
      );
    } finally {
      setIsGeocoding(false);
    }
          ...
          <View style={styles.statsRow}>
            <Text style={styles.distanceText}>
              {currentStep.distanceToNext > 0
                ? `${Math.round(currentStep.distanceToNext)}m to ${
                    currentStep.maneuverType === "arrive" ? "destination" : "turn"
                  }`
                : "Arriving"}
            </Text>
            {eta !== null && <Text style={styles.etaText}>ETA: {formatETA(eta)}</Text>}
          </View>
