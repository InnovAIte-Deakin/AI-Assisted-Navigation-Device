...
export default function ArtsScreen() {
  ...
  // Allow overrides from navigation, but fall back to Arts defaults
  const params = useLocalSearchParams<{ title?: string; distance?: string; clock?: string }>();
  const title = (params.title ?? "Arts").toString();
  const distance = (params.distance ?? "60 ft").toString();
  const clock = (params.clock ?? "1 o'clock").toString();
  ...
  const onSave = useCallback(() => {
    // TODO: persist with AsyncStorage / backend
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: GOLD, fontSize: 22, fontWeight: "800", letterSpacing: 1 },
