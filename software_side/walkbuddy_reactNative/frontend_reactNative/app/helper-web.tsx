...
import { collaborationService, normalizeCode, roomFor } from "@/src/utils/collaboration";
import { API_BASE } from "@/src/config";
import { Ionicons } from "@expo/vector-icons";

// Connection state machine
type ConnectionState = "idle" | "socket_connected" | "joined_room" | "webrtc_connecting" | "webrtc_connected" | "fallback_active";
...
export default function HelperWebScreen() {
  ...
  const [settingsSection, setSettingsSection] = useState<"account" | "privacy" | "help" | "about" | null>(null);
  ...
  const [sessionStatus, setSessionStatus] = useState<{ user_connected: boolean; expires_in: number } | null>(null);
  ...
  const confirmDeleteAccount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Platform.OS === "web" && typeof window !== "undefined") {
          window.alert("Your account has been deleted successfully. You will be redirected to the login page.");
        } else {
          Alert.alert(
            "Success",
  ...
  const handleSignup = async () => {
    ...
    if (!passwordValidation.isValid) {
      setAuthError(`Password requirements:\n${passwordValidation.errors.join("\n")}`);
      setIsLoadingAuth(false);
      return;
    }
    ...
    if (!termsAccepted) {
      setAuthError("You must accept the Terms and Conditions to sign up. Please check the box to continue.");
      setIsLoadingAuth(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (pathname && pathname !== "/helper-web" && !isNavigatingAwayRef.current) {
      console.log("[HelperWeb] Detected redirect attempt, preventing:", pathname);
    ...
    if (typeof window !== "undefined" && window.location.pathname !== "/helper-web") {
  ...
  useEffect(() => {
    if (Platform.OS === "web") {
      ...
      if (segments.length > 0) {
        const isRedirectingToTabs = segments.includes("(tabs)") || segments.includes("home");
        const isOnHelperWeb = pathname === "/helper-web" || segments[0] === "helper-web";
        
        if (isRedirectingToTabs && !isOnHelperWeb) {
          console.log("[HelperWeb] Preventing redirect to tabs/home, segments:", segments);
          router.replace("/helper-web");
        }
      }
    }
  ...
  const handleWebRTCOffer = async (sdp: RTCSessionDescriptionInit) => {
    try {
      ...
      pc.ontrack = (event) => {
        console.log("[HelperWeb] ✅ Received remote track:", event.track.kind, event.track.label);
        ...
        if (event.track.kind === "audio") {
          ...
          if (remoteAudioRef.current && remoteStream) {
              ...
              .catch((err) => {
                ...
                setTimeout(() => {
                  audioElement.play().catch((retryErr) => {
                    console.error("[HelperWeb] ❌ Audio play retry failed:", retryErr);
                  });
                }, 500);
              });
            ...
            console.error("[HelperWeb] ❌ Cannot attach audio - missing audioRef or stream:", {
        ...
        if (remoteStream && remoteVideoRef.current) {
          ...
          console.log("[HelperWeb] 📊 Remote stream has", videoTracks.length, "video track(s)");
          ...
          const checkVideoFrames = () => {
            if (remoteVideoRef.current && remoteVideoRef.current.videoWidth > 0) {
              ...
              console.log("[HelperWeb] ✅ Successfully attached remote MediaStream to video element");
              ...
              if (useFallbackModeRef.current) {
                console.log("[HelperWeb] 🔄 WebRTC track received, disabling fallback mode");
              ...
              console.log("[HelperWeb] 📤 Sending video_received acknowledgment");
          ...
          remoteVideoRef.current.play().then(() => {
            ...
            console.warn("[HelperWeb] ⚠️ Video play error (may be expected):", err);
          });
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          iceCandidateCount++;
          console.log("[HelperWeb] 📤 ICE candidate", iceCandidateCount, "generated:", event.candidate.candidate.substring(0, 50));
          collaborationService.sendWebRTCICE(event.candidate.toJSON());
        } else {
          console.log("[HelperWeb] ✅ ICE gathering complete. Sent", iceCandidateCount, "candidates");
        }
      };
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (state === "failed") {
          ...
          if (cameraFrame) {
            console.log("[HelperWeb] 🔄 WebRTC failed but frames received, switching to fallback mode");
      ...
      pc.oniceconnectionstatechange = () => {
        ...
        if (iceState === "failed") {
          ...
          // If we're receiving frames, switch to fallback mode instead of showing error
          if (cameraFrame) {
            console.log("[HelperWeb] 🔄 ICE failed but frames received, switching to fallback mode");
            setUseFallbackMode(true);
            setWebrtcError(null);
          } else {
            setWebrtcError("ICE connection failed. Check network connectivity.");
          }
      ...
      try {
        ...
        if (audioTracks.length > 0) {
          ...
          audioTracks.forEach((track) => {
            ...
            console.log("[HelperWeb] ✅ Added audio track:", track.label, "enabled:", track.enabled);
          });
        }
      } catch (err: any) {
      ...
      const trackTimeout = setTimeout(() => {
        if (!hasVideoTrackRef.current && peerConnectionRef.current) {
          console.warn("[HelperWeb] ⚠️ No video track received within 10 seconds");
          ...
          if (lastFrameDataUrl) {
            ...
            setWebrtcError("No video track received. Check ICE/signaling. Connection state: " + peerConnectionRef.current.connectionState);
          }
        }
      }, 10000);
  ...
  useEffect(() => {
    if (connectionState === "webrtc_connecting" && !webrtcStartTimeRef.current) {
      ...
      const webrtcTimeout = setTimeout(() => {
        if (connectionStateRef.current === "webrtc_connecting" && !hasVideoTrackRef.current) {
          console.log("[HelperWeb] ⏰ WebRTC timeout (8s), forcing fallback mode");
          if (lastFrameDataUrl) {
            setUseFallbackMode(true);
            useFallbackModeRef.current = true;
            setConnectionState("fallback_active");
            ...
            console.log("[HelperWeb] ⚠️ WebRTC timeout but no frames received yet");
          }
        }
      }, 8000);
  ...
  useEffect(() => {
    if (userConnected && !hasVideoTrackRef.current && !useFallbackModeRef.current) {
      console.log("[HelperWeb] 👤 User connected, expecting frames soon. Will switch to fallback mode when frames arrive.");
  ...
  useEffect(() => {
    if (cameraFrame) {
      ...
      console.log(`[FRAME] CHECKPOINT D: cameraFrame state updated (length=${frameDataUrl.length}), UI should render <img>`);
      console.log(`[FRAME] CHECKPOINT D: useFallbackMode=${useFallbackMode}, lastFrameDataUrl will be set`);
      ...
      if (!useFallbackMode && !hasVideoTrack) {
        console.log(`[FRAME] CHECKPOINT D: Activating fallback mode because frames are available`);
        setUseFallbackMode(true);
        useFallbackModeRef.current = true;
      }
    } else {
      ...
      if (!hasVideoTrack) {
        console.log(`[FRAME] CHECKPOINT D: cameraFrame state cleared, will show placeholder`);
        setLastFrameDataUrl(null);
      }
    }
  ...
  useEffect(() => {
    ...
    const unsubscribeConnected = collaborationService.onMessage("connected", (msg) => {
      const code = normalizeCode(msg.session_id || "");
      const room = roomFor(code);
      console.log(`[HelperWeb] ✅ Socket connected, received 'connected' event`);
      ...
      if ((msg as any).user_connected) {
        console.log("[HelperWeb] ✅ User is already connected (peer_connected event)");
    ...
    const unsubscribeUserConnected = collaborationService.onMessage("user_connected", () => {
      console.log("[HelperWeb] ✅ User connected event received (peer_connected)");
      setUserConnected(true);
      setConnectionState("joined_room");
      connectionStateRef.current = "joined_room";
    ...
    const unsubscribeUserDisconnected = collaborationService.onMessage("user_disconnected", () => {
    ...
    const unsubscribeCameraFrame = collaborationService.onMessage("frame", (msg) => {
      ...
      if (imageData) {
        ...
        if (currentCount === 1 || currentCount % 8 === 0) {
          console.log(`[FRAME] received {code: ${code.substring(0, 8)}, bytes: ${imageData.length}, ts: ${ts}}`);
          console.log(`[FRAME] framesReceived counter: ${currentCount}, room: ${room}`);
        }
        
        if (currentCount === 1) {
          console.log("[FRAME] ✅ First camera frame received! Switching to fallback mode immediately.");
        }
        ...
        if (!hasVideoTrackRef.current) {
          if (!useFallbackModeRef.current) {
            console.log("[FRAME] 🔄 Receiving frames but no WebRTC track, switching to fallback mode immediately");
            setUseFallbackMode(true);
            useFallbackModeRef.current = true;
            setConnectionState("fallback_active");
            connectionStateRef.current = "fallback_active";
        ...
        console.warn("[FRAME] ⚠️ Frame message received but no image data:", msg);
      }
    });
    ...
    frameStatsIntervalRef.current = setInterval(() => {
      ...
      if (count > 0) {
        console.log(`[FRAME] 📊 Helper stats (1s): framesReceived=${count}, fps=${count.toFixed(1)}, byteSize=${lastFrameDataUrl?.length || 0}`);
      }
      frameReceivedCountRef.current = 0; // Reset counter
    }, 1000);
    
    // Also listen for legacy camera_frame messages
    const unsubscribeLegacyFrame = collaborationService.onMessage("camera_frame" as any, (msg) => {
      if (msg.data) {
    ...
    const unsubscribeWebRTCOffer = collaborationService.onMessage("webrtc_offer", async (msg) => {
      const code = normalizeCode(msg.session_id || sessionId || "");
      const room = roomFor(code);
    ...
    const unsubscribeWebRTCAnswer = collaborationService.onMessage("webrtc_answer", async (msg) => {
      const code = normalizeCode(msg.session_id || sessionId || "");
      const room = roomFor(code);
    ...
    const unsubscribeWebRTCICE = collaborationService.onMessage("webrtc_ice", async (msg) => {
      const code = normalizeCode(msg.session_id || sessionId || "");
      const room = roomFor(code);
      console.log(`[HelperWeb] 📥 Received WebRTC ICE candidate (room: ${room})`);
      if (peerConnectionRef.current && msg.candidate) {
        try {
          ...
          if (peerConnectionRef.current.remoteDescription) {
            ...
            console.log("[HelperWeb] ⏳ Storing ICE candidate (waiting for remote description)");
          }
        } catch (error) {
  ...
  const handleCheckStatus = async () => {
    ...
    if (normalizedCode.length !== 8 || !/^[A-Z0-9]+$/.test(normalizedCode)) {
      Alert.alert("Invalid Code", "Session code must be exactly 8 alphanumeric characters.");
    ...
    try {
      ...
      console.log(`[HelperWeb] Checking status for code: "${sessionId}" → normalized: "${normalizedCode}"`);
      const status = await collaborationService.getSessionStatus(normalizedCode);
      setSessionStatus({
      ...
      if (errorMsg.includes("not found")) {
        alertMessage = "Session not found. Please check the code and make sure the user has created a session.";
      } else if (errorMsg.includes("expired")) {
        alertMessage = "Session expired. Please ask the user to create a new session.";
      } else if (errorMsg.includes("Invalid")) {
        alertMessage = "Invalid session code format. Must be 8 alphanumeric characters.";
      }
  ...
  const handleJoinSession = async () => {
    ...
    console.log(`[HelperWeb] 🚀 handleJoinSession called with code: "${sessionId}" → normalized: "${normalizedCode}"`);
    ...
    try {
      ...
      console.log(`[HelperWeb] 📍 Joining session with code: "${sessionId}" → normalized: "${normalizedCode}"`);
      ...
      // Connect as guide
      console.log(`[HelperWeb] 📍 Calling collaborationService.connect(${normalizedCode}, "guide")...`);
      await collaborationService.connect(normalizedCode, "guide");
      console.log(`[HelperWeb] ✅ collaborationService.connect() completed successfully`);
      
      // Send helper name after connection
      if (helperName) {
        console.log(`[HelperWeb] 📤 Sending helper name: "${helperName}"`);
        collaborationService.sendMessage("helper_info" as any, { helper_name: helperName });
      }
    } catch (err) {
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>
          This helper interface is only available on web browsers.
          Please open this page in a browser.
        </Text>
      </View>
    );
  }
    ...
    return (
      <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerSubtitle}>Help someone navigate in real-time</Text>
        </View>
        ...
        <View style={styles.authContainer}>
          ...
          <View style={styles.authToggle}>
            <Pressable
              style={[styles.authToggleButton, !showSignup && styles.authToggleButtonActive]}
              onPress={() => {
                setShowSignup(false);
                setAuthError(null);
              }}
            >
              <Text style={[styles.authToggleText, !showSignup && styles.authToggleTextActive]}>
                Login
              </Text>
            </Pressable>
            <Pressable
          ...
          {!showSignup ? (
            <View style={styles.authForm}>
              ...
              <Pressable
                style={[styles.authSubmitButton, isLoadingAuth && styles.authSubmitButtonDisabled]}
                onPress={handleLogin}
            ...
            <View style={styles.authForm}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setSignupData({ ...signupData, age: text.replace(/[^0-9]/g, "") })}
              ...
              <TextInput
                ...
                onChangeText={(text) => setSignupData({ ...signupData, emergency_contact_name: text })}
              ...
              <TextInput
                ...
                onChangeText={(text) => setSignupData({ ...signupData, emergency_contact_phone: text })}
              ...
              <TextInput
                ...
                onChangeText={(text) => setSignupData({ ...signupData, experience_level: text })}
              ...
              <TextInput
                ...
                onChangeText={(text) => {
                  ...
                  if (authError && authError.includes("Password requirements")) {
                    setAuthError(null);
                  }
                }}
              ...
              <Pressable
                style={[
                  ...
                  (isLoadingAuth || !termsAccepted) && styles.authSubmitButtonDisabled
                ]}
    ...
    <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerDescription}>
          No app download required. Just enter the session code to start helping.
        </Text>
        {/* Logout Button */}
        <Pressable
      ...
      {!isConnected ? (
        <View style={styles.joinContainer}>
          ...
          {sessionStatus && (
            <View style={styles.statusContainer}>
              <View style={styles.statusRow}>
                <View style={[styles.statusDot, sessionStatus.user_connected && styles.statusDotActive]} />
                <Text style={styles.statusText}>
                  {sessionStatus.user_connected ? "User Connected" : "Waiting for user"}
                </Text>
              </View>
          ...
          <View style={styles.buttonRow}>
            <Pressable
              style={[styles.secondaryButton, isCheckingStatus && styles.secondaryButtonDisabled]}
        ...
        <View style={styles.helperContainer}>
          ...
          <View style={styles.statusBar}>
            ...
            />
            <Text style={styles.statusText}>
              {userConnected && (hasVideoTrack || useFallbackMode || lastFrameDataUrl)
            ...
            {useFallbackMode && (
              <Text style={styles.statusSubtext}>Using reliable frame streaming</Text>
            )}
            {/* Audio Status */}
            {userReceivedAudio && (
              <View style={styles.audioStatus}>
                <Ionicons name="volume-high" size={16} color="#4CAF50" />
                <Text style={styles.audioStatusText}>Receiving audio from user</Text>
              </View>
            )}
                ...
                {Platform.OS === "web" && (
                  <>
                    ...
                    <audio
                      ...
                      style={{ display: "none" }}
                      onLoadedMetadata={() => {
                        console.log("[HelperWeb] ✅ Audio element metadata loaded");
                        if (remoteAudioRef.current) {
                      ...
                      onCanPlay={() => {
                        ...
                        if (remoteAudioRef.current && remoteAudioRef.current.paused) {
                          remoteAudioRef.current.play().catch((err) => {
                            console.warn("[HelperWeb] ⚠️ Auto-play prevented, user interaction required:", err);
                          });
                        }
                      }}
                      onError={(e) => {
              ...
              <>
                ...
                {Platform.OS === "web" ? (
                  <img
                    ...
                    onLoad={() => {
                      console.log("[FRAME] CHECKPOINT D: ✅ Frame image loaded in UI (img src set, placeholder hidden)");
                      console.log("[FRAME] CHECKPOINT D: Image dimensions:", {
                        naturalWidth: (document.querySelector('img[alt="User camera feed"]') as HTMLImageElement)?.naturalWidth,
                        naturalHeight: (document.querySelector('img[alt="User camera feed"]') as HTMLImageElement)?.naturalHeight,
                      });
                    }}
                    onError={(e) => {
                      console.error("[FRAME] CHECKPOINT D: ❌ Frame image failed to load:", e);
                      console.error("[FRAME] CHECKPOINT D: Failed image src length:", lastFrameDataUrl.length);
                      console.error("[FRAME] CHECKPOINT D: Failed image src preview:", lastFrameDataUrl.substring(0, 100));
                    }}
                    style={{
                      width: "100%",
                      height: "100%",
                  ...
                  <Image
                    ...
                    resizeMode="cover"
                    onLoad={() => {
                      console.log("[FRAME] CHECKPOINT D: ✅ Frame image loaded in UI (Image component)");
                    }}
                    onError={(e) => {
                      console.error("[FRAME] CHECKPOINT D: ❌ Frame image failed to load:", e);
                    }}
            ...
            <View style={styles.audioControls}>
              {hasAudioTrack ? (
                <Pressable
                  onPress={() => {
                    if (localAudioTrackRef.current) {
                      ...
                      console.log("[HelperWeb] 🎤 Microphone", newMutedState ? "unmuted" : "muted");
                    }
                  }}
                  style={[
                    styles.muteButton,
                ...
                <Pressable
                  onPress={async () => {
                    try {
                      console.log("[HelperWeb] 🎤 Manually requesting microphone access...");
                      const audioStream = await navigator.mediaDevices.getUserMedia({
                        audio: {
                          echoCancellation: true,
                          noiseSuppression: true,
                      ...
                      if (audioTracks.length > 0) {
                        ...
                        if (peerConnectionRef.current) {
                          audioTracks.forEach((track) => {
                            ...
                            console.log("[HelperWeb] ✅ Added audio track:", track.label);
                          });
                        }
                        console.log("[HelperWeb] ✅ Microphone access granted");
                      }
                    } catch (err: any) {
                      console.error("[HelperWeb] ❌ Microphone access failed:", err);
                      Alert.alert("Microphone Access", "Failed to access microphone. Please check your browser permissions.");
                    }
                  }}
                ...
                <Pressable
                  onPress={async () => {
                    try {
                      ...
                      console.error("[HelperWeb] ❌ Microphone access failed:", err);
                      setMicrophonePermission("denied");
                      Alert.alert("Microphone Access", "Microphone access was denied. Please allow microphone access in your browser settings.");
                    }
                  }}
                  style={styles.requestMicButton}
                >
          ...
          <View style={styles.guidanceContainer}>
            ...
            <View style={styles.guidanceInputRow}>
              ...
              <Pressable
                style={[
                  styles.sendButton,
                  (!userConnected || !guidanceText.trim()) && styles.sendButtonDisabled,
                ]}
          ...
          <View style={styles.modalContainer}>
            ...
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {settingsSection === "account" ? "Account" :
                 settingsSection === "privacy" ? "Privacy & Policy" :
                 settingsSection === "help" ? "Help Center" :
            ...
            {!settingsSection && (
              <View style={styles.settingsMenu}>
                ...
                >
                  <Ionicons name="lock-closed-outline" size={24} color="#F9A826" />
                ...
                >
                  <Ionicons name="help-circle-outline" size={24} color="#F9A826" />
                ...
                {helperData ? (
                  <>
                    <View style={styles.accountSection}>
                      ...
                      <Text style={styles.accountValue}>{helperData.name || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Age</Text>
                      <Text style={styles.accountValue}>{helperData.age || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Email</Text>
                      <Text style={styles.accountValue}>{helperData.email || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Phone Number</Text>
                      <Text style={styles.accountValue}>{helperData.phone || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Address</Text>
                      <Text style={styles.accountValue}>{helperData.address || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Emergency Contact Name</Text>
                      <Text style={styles.accountValue}>{helperData.emergency_contact_name || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Emergency Contact Phone</Text>
                      <Text style={styles.accountValue}>{helperData.emergency_contact_phone || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Experience Level</Text>
                      <Text style={styles.accountValue}>{helperData.experience_level || "N/A"}</Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Account Created</Text>
                      <Text style={styles.accountValue}>
                        {helperData.created_at ? new Date(helperData.created_at).toLocaleDateString() : "N/A"}
                      </Text>
                    </View>
                    <View style={styles.accountSection}>
                      <Text style={styles.accountLabel}>Last Login</Text>
                      <Text style={styles.accountValue}>
                        {helperData.last_login ? new Date(helperData.last_login).toLocaleDateString() : "N/A"}
                      </Text>
                    </View>
                    <Pressable
            ...
            {settingsSection === "privacy" && (
              <ScrollView style={styles.settingsContent}>
                ...
                <Text style={styles.sectionText}>
                  WalkBuddy Helper is committed to protecting your privacy. This Privacy Policy explains how we collect, use, and safeguard your personal information.
                </Text>
                <Text style={styles.sectionSubtitle}>Information We Collect</Text>
                <Text style={styles.sectionText}>
                  We collect information you provide during registration, including your name, email, phone number, address, and emergency contact details.
                </Text>
                <Text style={styles.sectionSubtitle}>How We Use Your Information</Text>
                <Text style={styles.sectionText}>
                  Your information is used to facilitate the assistance service, connect helpers with users, and ensure the safety and security of our platform.
                </Text>
            ...
            {settingsSection === "help" && (
              <ScrollView style={styles.settingsContent}>
                ...
                <Text style={styles.sectionText}>
                  Need assistance? We're here to help! Contact our support team for any questions or issues you may have.
                </Text>
                ...
                <Pressable
                  ...
                  onPress={() => {
                    ...
                    const subject = encodeURIComponent("WalkBuddy Helper - Support Request");
                ...
                >
                  ...
                  <View style={styles.helpContactTextContainer}>
                    ...
                    <Text style={styles.helpContactSubtitle}>support@walkbuddy.com</Text>
                  </View>
                  <Ionicons name="chevron-forward" size={20} color="#AAA" />
                </Pressable>
                ...
                <Pressable
                  style={[
                    ...
                    (!supportMessage.trim() || isSendingSupport) && styles.supportSendButtonDisabled
                  ]}
                  onPress={async () => {
                    ...
                    try {
                      ...
                      if (Platform.OS === "web" && typeof window !== "undefined") {
                        ...
                        setSupportMessage("");
                        if (Platform.OS === "web" && typeof window !== "undefined") {
                          window.alert("Your message has been prepared. Please send it from your email client.");
                        } else {
                      ...
                      if (Platform.OS === "web" && typeof window !== "undefined") {
                        window.alert("Unable to open email client. Please email support@walkbuddy.com directly.");
                      } else {
                        Alert.alert("Error", "Unable to open email client. Please email support@walkbuddy.com directly.");
                      }
                    } finally {
                ...
                >
                  {isSendingSupport ? (
                    ...
                    <>
                      ...
                      <Text style={styles.supportSendButtonText}>Send Message</Text>
                    </>
                  )}
                </Pressable>
                ...
                <View style={styles.faqItem}>
                  ...
                  <Text style={styles.faqAnswer}>
                    Enter the 8-character session code provided by the user in the "Enter Session Code" field and click "Join Session".
                  </Text>
                </View>

                <View style={styles.faqItem}>
                  <Text style={styles.faqQuestion}>I can't see the user's camera feed</Text>
                  <Text style={styles.faqAnswer}>
                    Make sure the user has enabled their camera and is connected. Try refreshing the page or ask the user to check their connection.
                  </Text>
                </View>

                <View style={styles.faqItem}>
                  <Text style={styles.faqQuestion}>How do I send guidance to the user?</Text>
                  <Text style={styles.faqAnswer}>
                    Type your guidance message in the "Send Guidance" field and click "Send". The user will hear your message spoken aloud.
                  </Text>
                </View>

                <View style={styles.faqItem}>
                  <Text style={styles.faqQuestion}>Can I use my microphone?</Text>
                  <Text style={styles.faqAnswer}>
                    Yes! Click the "Enable Microphone" button to allow audio communication with the user. Make sure to grant microphone permissions when prompted.
                  </Text>
                </View>
                ...
                <View style={styles.faqItem}>
                  ...
                  <Text style={styles.faqAnswer}>
                    Go to Settings → Account → Delete Account. This action cannot be undone, so make sure you want to permanently delete your account.
                  </Text>
                </View>
              </ScrollView>
            )}
            ...
            {settingsSection === "about" && (
              <ScrollView style={styles.settingsContent}>
                ...
                <Text style={styles.sectionText}>
                  WalkBuddy Helper is a real-time assistance platform designed to help people navigate and receive guidance from trusted helpers.
                </Text>
                <Text style={styles.sectionSubtitle}>Our Mission</Text>
                <Text style={styles.sectionText}>
                  To provide accessible, real-time assistance to those who need help navigating their environment, connecting helpers with users in a safe and secure manner.
                </Text>
                ...
                <Text style={styles.sectionText}>Mekong Inclusive Ventures</Text>
              </ScrollView>
            )}
