...
public class CameraNavigationActivity extends AppCompatActivity {
    ...
    // Nearby obstacle alert timing
    private long lastObstacleAlertTime = 0;
    private static final long OBSTACLE_ALERT_INTERVAL = 3000; // 3 seconds
    ...
    private String processYOLOv8Output(float[] output, int numClasses, int numAnchors) {
        ...
        // Trigger voice and vibration alert for nearby obstacles
        checkNearbyObstacleAndAlert(finalDetections);
    ...
    private void checkNearbyObstacleAndAlert(List<DetectionOverlay.Detection> detections) {
        if (detections == null || detections.isEmpty()) {
            return;
        }

        DetectionOverlay.Detection closestDetection = null;

        for (DetectionOverlay.Detection detection : detections) {
            if (isObjectNearby(detection.boundingBox)) {
        ...
        if (currentTime - lastObstacleAlertTime > OBSTACLE_ALERT_INTERVAL) {
            String warningMessage = "Warning, " + closestDetection.className + " detected nearby";

            Log.d(TAG, "Nearby obstacle alert: " + warningMessage);

            if (announcer != null) {
                announcer.speak(warningMessage);
            }

            triggerVibrationAlert();

            lastAnnouncement = warningMessage;
            txtLastAnnouncement.setText("Last: " + lastAnnouncement);

            Toast.makeText(this, warningMessage, Toast.LENGTH_SHORT).show();

            lastObstacleAlertTime = currentTime;
        }
    }

    private boolean isObjectNearby(RectF boundingBox) {
        if (boundingBox == null) {
            return false;
        }
        ...
        // Bigger boxes usually mean the object is closer to the camera.
        return boxArea > 90000;
    }
